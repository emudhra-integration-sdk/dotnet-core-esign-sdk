...
using static eSignASPLibrary.eSign;
    ...
    public class eSignInternal
    {
        ...
        public eSignServiceReturn GetGateWayParam(List<eSignInput> eSignInputs, string signerID, string transactionID, string resposeURL, string redirectUrl, string TempFolderPath, eSign.eSignAPIVersion eSignAPIVersion = eSignAPIVersion.V3, eSign.AuthMode authMode = AuthMode.OTP, bool isLTVRequired = true)
        {
            ...
            try
            {
                ...
                {
                    oreturn = new eSignServiceReturn() { ASPID = ASPID, TransactionID = TransactionID, ErrorCode = "ESS-112", ErrorMessage = "Must contain more than one input" };
                    return oreturn;
                }
                int count = 1;
                        ...
                        if (eSignInput.docType.Equals(DocType.Pdf))
                        {
                            ...
                            switch (eSignInput.Coordinates)
                            {
                                case Coordinates.Top_Left:
                                    cuCoordinates = "25,725,150,785";
                                    break;
                                case Coordinates.Top_Center:
                                    cuCoordinates = "225,725,350,785";
                                    break;
                                case Coordinates.Top_Right:
                                    cuCoordinates = "425,725,550,785";
                                    break;
                                case Coordinates.Middle_Left:
                                    cuCoordinates = "7,485,124,423";
                                    break;
                                case Coordinates.Middle_Right:
                                    cuCoordinates = "468,483,590,421";
                                    break;
                                case Coordinates.Middle_Center:
                                    cuCoordinates = "243,484,365,422";
                                    break;
                                case Coordinates.Bottom_Left:
                                    cuCoordinates = "7,65,125,3";
                                    break;
                                case Coordinates.Bottom_Center:
                                    cuCoordinates = "248,65,370,3";
                                    break;
                                case Coordinates.Bottom_Right:
                                    cuCoordinates = "468,66,590,4";
                                    break;
                            }
                            Rectangle rect = null;
                            if (eSignInput.PageTobeSigned == PageToBeSigned.PAGE_LEVEL)
                            {
                                ...
                                for (int i = 0; i < Cordinatespagelevel.Length; i++)
                                {
                                    ...
                                    {
                                        var pageList = GetPageNumber(Pagelevel[0], totalPages);
                                ...
                                signatureAppearance.SetVisibleSignature(RectangleList.ToArray(), PageList.ToArray(), TransactionID);
                            }
                            else
                            {
                            ...
                            int position = (int)signatureAppearance.exclusionLocations[PdfName.CONTENTS].Position;
                ...
                try
                {
                    // ResponseXML = eSignUtility.HttpsWebClientSendRequest(eSignSettings.eSignURL, URLEncodedSignedXML);
        ...
        private List<int> GetPageNumber(string pageNumber, int totalPages)
        {
